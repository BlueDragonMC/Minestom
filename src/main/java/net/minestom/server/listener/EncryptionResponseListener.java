...
public final class EncryptionResponseListener {

    private static final ConnectionManager CONNECTION_MANAGER = MinecraftServer.getConnectionManager();
    private static final Gson GSON = new Gson();

    public static void listener(EncryptionResponsePacket packet, PlayerConnection connection) {
        ...
        AsyncUtils.runAsync(() -> {
            ...
            if (!Arrays.equals(socketConnection.getNonce(), getNonce(packet))) {
                MinecraftServer.LOGGER.error("{} tried to login with an invalid nonce!", loginUsername);
                return;
            }

            final byte[] digestedData = MojangCrypt.digestData("", MojangAuth.getKeyPair().getPublic(), getSecretKey(packet));
            ...
            client.sendAsync(request, HttpResponse.BodyHandlers.ofString()).whenComplete((response, throwable) -> {
                ...
                try {
                    ...
                    socketConnection.setEncryptionKey(getSecretKey(packet));
    ...
    private static SecretKey getSecretKey(EncryptionResponsePacket packet) {
        return MojangCrypt.decryptByteToSecretKey(MojangAuth.getKeyPair().getPrivate(), packet.sharedSecret());
    }

    private static byte[] getNonce(EncryptionResponsePacket packet) {
        return MojangAuth.getKeyPair().getPrivate() == null ?
                packet.verifyToken() : MojangCrypt.decryptUsingKey(MojangAuth.getKeyPair().getPrivate(), packet.verifyToken());
    }
